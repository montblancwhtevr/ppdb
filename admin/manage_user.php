...
<div class="panel-body">
	<?php $jml =mysqli_num_rows(mysqli_query($con,"SELECT * FROM tb_siswa"));?>
	<?php $jml1 =mysqli_num_rows(mysqli_query($con,"SELECT * FROM tb_user"));?>
<br>
...
<tbody>
	<?php
	$sql=mysqli_query($con,"select * from tb_siswa") or die(mysqli_error());
	$no=1;
...
Jumlah User <span class="badge"><?php echo "".$jml1." ";?></span>
	...
	<table class="table table-striped table table-condensed table table-hover">
		...
		<tbody>
			<?php
			$sql=mysqli_query($con,"SELECT * FROM tb_user") or die(mysqli_error());
			$no=1;
			while ($data=mysqli_fetch_array($sql)) {
			 	# code...
...
$carikode = mysqli_query($con,"select max(kode_user) from tb_user") or die(mysqli_error());
...
?>
    ...
    <div class="modal-body">
        ...
        <br>
         <div  class="from-group">
			...
			<label for="radio_30">Admin/Panitia PPDB</label> <br>
	...
	$level = @$_POST['level'];
      ...
      if ($simpan_data) {
        ...
        if ($pindah) {
          mysqli_query($con,"insert into tb_user values ('$kode_user','$username','$password','$pass','$nama_lengkap','$level','$nama_gambar')") or die (mysqli_error());
           ?>
